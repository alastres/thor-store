...
export default function DarkVeil({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      // Scale DPR down for performance — canvas renders at lower res but displays at full size
      dpr: Math.min(window.devicePixelRatio, 2) * resolutionScale,
      canvas,
    })

    const gl = renderer.gl
    // OGL's Renderer enables depth test + back-face culling by default. The
    // fullscreen Triangle's winding gets backface-culled under that default,
    // so the mesh never rasterizes at all — canvas just shows the clear color.
    // OGL's renderer re-applies gl.enable(CULL_FACE)/DEPTH_TEST per draw call
    // from the Program's own state, so disabling on `gl` directly doesn't stick —
    // it must be set via the Program options below (cullFace: false, depthTest: false).
    // OGL tracks its own width/height internally and re-applies gl.viewport()
    // from that stale state on every render() call, which can desync from the
    // canvas's real drawing-buffer size after repeated resizes (observed: stuck
    // at an early 150x75 viewport while canvas.width/height grew to 680x290).
    // Force every viewport call to always match the actual canvas buffer.
    const nativeViewport = gl.viewport.bind(gl)
    gl.viewport = () => nativeViewport(0, 0, canvas.width, canvas.height)
    ...
    const resize = () => {
      ...
      if (w < 10 || h < 10) return
      // setSize with full dims — dpr already scaled for lower render resolution
      renderer.setSize(w, h)
      // Override OGL's inline px style so canvas stretches to fill parent
